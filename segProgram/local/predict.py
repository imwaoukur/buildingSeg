...
def predict_img(img_path, resolution):
    ...
    with torch.no_grad():
        ...
        pre_img = cv2.drawContours(img_bgr, contours=contours, contourIdx=-1, color=(64, 224, 208), thickness=3)
        image_pil = Image.fromarray(cv2.cvtColor(pre_img, cv2.COLOR_BGR2RGB))
        image_pil.save(os.path.join(PROJECT_ROOT, 'local/pre_img.png'))
        return space
